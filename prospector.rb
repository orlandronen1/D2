...
class Prospector
  ...
  def look_for_rubies
    ...
    (last_ruby_haul + last_fake_ruby_haul).positive?
  end
  ...
  def print_findings
    if (last_ruby_haul + last_fake_ruby_haul).zero?
      ...
      if last_ruby_haul >= 1 && last_fake_ruby_haul >= 1
        print ' and'
      end

      # Print info on fake rubies found, if any
      if last_fake_ruby_haul.zero?
        print ''
